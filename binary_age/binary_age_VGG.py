...
X_train = np.reshape(X_train, (X_train.shape[0], 256, 256, 3))
X_val = np.reshape(X_val, (X_val.shape[0], 256, 256, 3))
X_test = np.reshape(X_test, (X_test.shape[0], 256, 256, 3))
X_inference = np.reshape(X_inference, (X_inference.shape[0], 256, 256, 3))
...
for example in X_inference:
    ...
    # Make prediction
    try:
        prediction.append(classifier.predict(features))
    except:
        prediction.append(classifier.predict(features.reshape(1, 7*7*512)))


prediction = np.array(prediction)


prediction[prediction < 0.5] = 'old'
prediction[prediction > 0.5] = 'young'
...
with open('./result_logs/inference_VGG.csv', 'w') as csvfile:
    ...
    for i, file in enumerate(list(range(1,101)):
        filewriter.writerow(['{0}.png'.format(file), prediction[i]])
